...
The business risk is concrete: a breach traced to an unreviewed deploy, a SOX deficiency from missing approvals, or a HIPAA finding from a deploy that exposed PHI to a non-production region. All three are preventable with disciplined pipelines.
...
- **Artifact promotion**: build once into a regulated container registry (ECR with image signing via Cosign, plus SBOM via Syft). Promote by re-tagging, not rebuilding.
...
| Segregation of duties on approval | CC1.4 | A.5.3 | AC-5 |
| Build integrity and artifact signing | CC7.1 | A.8.28, A.8.30 | SA-10, SR-4 |
| Use of short-lived credentials | CC6.1 | A.5.16 | IA-5, AC-2 |
| Evidence retention for changes | CC7.2 | A.8.15 | AU-2, AU-12 |
